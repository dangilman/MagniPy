...
def compute_fluxratio_distributions(halo_model='', model_args={},
                                    ...
                                    single_background=False,n_restart=1, pso_conv_mean = 100, toggle_interp = 1e+6):
    ...
    if method=='lenstronomy':
        ...
        while len(fit_fluxes)<Ntotal:
            ...
            model_data, system = solver.optimize_4imgs_lenstronomy(datatofit=data,macromodel=start_macromodel,realizations=use_real,
                                   multiplane=multiplane,n_particles = 50, simplex_n_iter=400, n_iterations = 300, source_size_kpc=source_size_kpc,
                                   optimize_routine = 'fixed_powerlaw_shear',verbose=True, pso_convergence_mean=pso_conv_mean,
                                         re_optimize=False, particle_swarm=True, restart=n_restart,
                                           single_background=single_background, toggle_interp=toggle_interp, interp_res=5e-5,
                                                                   interp_range=0.025)
            print('high res')
