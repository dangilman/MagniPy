...
def compute_fluxratio_distributions(halo_model='', model_args={},
    ...
    if method=='lenstronomy':
        ...
        while len(fit_fluxes)<Ntotal:
            ...
            _, init = solver.optimize_4imgs_lenstronomy(datatofit=data, macromodel=start_macromodel,
                                                                   realizations=use_real,
                                                                   multiplane=multiplane, n_particles=50,
                                                                   n_iterations=300, source_size_kpc=source_size_kpc,
                                                                   optimize_routine='fixed_powerlaw_shear',
                                                                   verbose=False,
                                                                   re_optimize=False, particle_swarm=True, restart=2,
                                                                   single_background=False)

            model_data, system = solver.optimize_4imgs_lenstronomy(datatofit=data,macromodel=init[0],realizations=use_real,
                                   ...
                                   optimize_routine = 'fixed_powerlaw_shear',verbose=True,
                                         re_optimize=True, particle_swarm=False, restart=2,
