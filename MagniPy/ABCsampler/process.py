...
def resample_chain(a0_area=None, logmhm=None, src_size=None, LOS_norm=1.0, errors = [0, 0.04],N_pert=1,
                   process_only = False, SIE_gamma_mean=2.05,
                   SIE_gamma_sigma=0.06, logmhm_sigma = 0.05,
...
def resample_sys(num, process_only):
    ...
    if num == 1:
        resample_chain(a0_area=1.6, logmhm=4.95, src_size=src_mean, LOS_norm=1.1, errors=errors,
                   N_pert=20, process_only=process_only, name='WDM_7.7_sigma0.012_srcsize35',
        ...
        resample_chain(a0_area=1.8, logmhm=4.95, src_size=src_mean, LOS_norm=1.1, errors=errors,
                       N_pert=20, process_only=process_only, name='WDM_7.7_sigma0.012_srcsize35',
                       which_lens_indexes=np.arange(1, 51), ending = '_exact')
        ...
        resample_chain(a0_area=1.3, logmhm=7.7, src_size=src_mean, LOS_norm=1.1, errors=errors,
                       N_pert=20, process_only=process_only, name='WDM_7.7_sigma0.012_srcsize35',
                       which_lens_indexes=np.arange(1, 51), ending = '_exact')
    elif num == 6:
        resample_chain(a0_area=1.3, logmhm=7.7, src_size=src_mean, LOS_norm=1, errors=errors,
                       N_pert=20, process_only=process_only, name='WDM_7.7_sigma0.012_srcsize35',
                       which_lens_indexes=np.arange(1, 51))
    if num == 7:
        resample_chain(a0_area=0, logmhm=4.95, src_size=src_mean, LOS_norm=1, errors=errors,
                   N_pert=20, process_only=process_only, name='WDM_7.7_sigma0.013_srcsize35',
                   which_lens_indexes=np.arange(1, 51))
...
#exit(1)
#process_samples('WDM_7.7_sigma0.012_srcsize35', np.arange(1,51), 1, [0])
for k in range(1, 8):
