...
def build_dictionary_list(paramnames=[],values=[],dictionary_to_duplicate=None,N=int):
    ...
    for i in range(0,N):
        ...
        if dictionary_to_duplicate is not None:
            new_dictionary = dictionary_to_duplicate
        else:
            new_dictionary = {}
...
def runABC(inputfile_path='',Nsplit=1000):
    ...
    if len(chain_keys_to_vary_sparse.keys())!=0:
        prior_sparse = ParamSample(params_to_vary=chain_keys_to_vary_sparse,Nsamples=int(np.shape(samples)[0]))
        sparse_samples = prior_sparse.sample(scale_by='Nsamples')
        param_names_tovary += prior_sparse.param_names
        samples = np.hstack((samples,sparse_samples))
    ...
    macromodel = initialize_macromodel(macromodel_start, data2fit=datatofit, method=chainkeys['solve_method'],
                                       ...
                                       source_size=chainkeys['source_size'], outfilename=chainkeys['outfile'],
                                       multiplane=chainkeys['multiplane'], raytrace_with=chainkeys['raytrace_with'])

    macromodels = []

    if 'SIE_gamma' in param_names_tovary:
    ...
    assert len(run_commands)%N_each==0,'run_commands length '+str(len(run_commands))+\
                                       ' and N_each length '+str(N_each)+' not compatible.'

    Nsplit = len(run_commands)*N_each**-1

    chain_data = []
    for i in range(0, int(Nsplit)):
        chain_data += reoptimize_with_halos(start_macromodels=macromodels,
                                            realizations=realizations[i * N_each:(i + 1) * N_each],
                                            ...
                                            write_to_file=False,outfilepath=run_commands[i]['outfile'],
...
chain_keys['sampler']['outfile'] = 'tester'
...
params_to_vary['logmhm'] = {'prior_type':'Uniform','low':6,'high':10,'steps':21,'sample_full':True}
params_to_vary['SIE_gamma'] = {'prior_type':'Gaussian','mean':2,'sigma':0.05,'positive_definite':True,'sample_full':False,'sort_ascending':True}
