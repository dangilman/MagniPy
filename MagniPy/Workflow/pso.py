...
class PSOoptimizer(object):
    ...
    def __init__(self, func, low, high, particleCount=25, verbose=False,
        ...
        if self._bounds is not None:
            assert len(self._bounds) == len(low)
    ...
    def _initSwarm(self):
        ...
        for _ in range(self._particleCount):
            swarm.append(Particle(numpy.random.uniform(self._low, self._high, size=self._paramCount),
                                  numpy.zeros(self._paramCount)))
    ...
    def _sample(self, maxIter=1000, c1=1.193, c2=1.193, lookback=0.25, standard_dev=None):
        ...
        while True:
            ...
            if hasattr(self._func, 'is_converged'):
            ...
            for particle in self.swarm:
                ...
                while True:
                    ...
                    newvel = w * particle.velocity
                    cog_vel = c1 * numpy.random.uniform(0, 1, size=self._paramCount) * (
                                particle.pbest.position - particle.position)
                    soc_vel = c2 * numpy.random.uniform(0, 1, size=self._paramCount) * (
                                self._gbest.position - particle.position)
                    #particle.velocity = part_vel + cog_vel + soc_vel
                    vprop = newvel + cog_vel + soc_vel
                    newpos = particle.position + vprop
                    #particle.position = particle.position + particle.velocity

                    if not self._out_of_bounds(newpos):
                        particle.position = newpos
                        particle.velocity = vprop
                        break
                    else:
                        w *= -1
    ...
    def _out_of_bounds(self, particle_position):
        ...
        for i, bound in enumerate(self._bounds):
            ...
            if particle_position[i] < bound[0] or particle_position[i] > bound[1]:
                return True
