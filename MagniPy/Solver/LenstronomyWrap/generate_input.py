...
from MagniPy.util import shr_convert
...
class LenstronomyWrap:
    ...
    def assemble(self,system,include_shear=False):
        ...
        for deflector in system.lens_components:
            ...
            lens_model_list.append(deflector.lenstronomy_name)
            lens_model_params.append(deflector.lenstronomy_args)

            if deflector.has_shear and include_shear:

                lens_model_list.append('EXTERNAL_SHEAR')
                e1,e2 = shr_convert(deflector.shear,deflector.shear_theta,polar_to_cart=True)
                lens_model_params.append({'e1':e1,'e2':e2})

        self.lens_model_list, self.lens_model_params = lens_model_list,lens_model_params

    def update_lensparams(self, system_index, newkwargs):

        self.lens_model_params[system_index] = newkwargs
    ...
    def solve_leq(self,xsrc,ysrc):
        ...
        lensEquationSolver = LensEquationSolver(lens_model_list=self.lens_model_list)
        x_image,y_image =  lensEquationSolver.image_position_from_source(kwargs_lens=self.lens_model_list, sourcePos_x=xsrc, sourcePos_y=ysrc,
    ...
    def optimize_lensmodel(self,x_image,y_image):
        ...
        solver4Point = Solver4Point(lens_model_list=self.lens_model_list, decoupling=True)
